...
def _make_filter_dependency(resource: type[Resource]) -> DependsType | None:
    fc = resource.filter_class
    if fc is None:
        return None
    return Depends(fc)
...
def _build_sync_routes(
    ...
    filter_dep = _make_filter_dependency(resource)
...
def _build_async_routes(
    ...
    async def update_item(id: int, data: UpdateSchema, session=Depends(get_session)):  # type: ignore[valid-type]
        obj = await service.async_update(
            session, id, data.model_dump(exclude_unset=True)
